...
function Schedule() {
	...
	async function getCurrentWeek() {
		try {
			const connect = await fetch("http://0.0.0.0:3000/current_date")
	...
	useEffect(() => {
		...
		async function loadingSchedule() {
			try {
				const allEduGroups = await fetch("http://0.0.0.0:3000/groups")
	...
	async function getScheduleData() {
		try {
			...
			const server = await fetch("http://0.0.0.0:3000/schedule?group="+current_group)
